...
class BRFasterRcnnTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale, attack=False):
        ...
        #if not features.sum()[0] == 0:    IndexError invalid index of a 0-dim tensor
    ...
    def save(self, save_optimizer=False, save_path=None, save_rcnn=False, **kwargs):
        ...
        if save_path is None:
            ...
            save_path = '/content/drive/My Drive/img_attack_with_attention/checkpoints/faterrcnn_full_%s' % timestr
            if not self.attack_mode:
                for k_, v_ in kwargs.items():
                    save_path += '%s' % v_
            if self.attacker is not None:
                self.attacker.save('/content/drive/My Drive/img_attack_with_attention/checkpoints/attack_%s_%d.path' % (timestr, kwargs['epochs']))
        if save_rcnn:
